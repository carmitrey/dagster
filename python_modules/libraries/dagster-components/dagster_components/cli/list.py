...
from dagster_components.core.defs import RemoteDefinitionMetadata
...
def list_definitions_command(ctx: click.Context, **other_opts: object) -> None:
    ...
    all_defs: list[RemoteDefinitionMetadata] = []

    for asset in repo_def.asset_graph.get_all_asset_keys():
        all_defs.append(RemoteDefinitionMetadata(type="asset", name=asset.to_user_string()))
    for job in repo_def.get_all_jobs():
        if not is_reserved_asset_job_name(job.name):
            all_defs.append(RemoteDefinitionMetadata(type="job", name=job.name))
    for schedule in repo_def.schedule_defs:
        all_defs.append(RemoteDefinitionMetadata(type="schedule", name=schedule.name))
    for sensor in repo_def.sensor_defs:
        all_defs.append(RemoteDefinitionMetadata(type="sensor", name=sensor.name))
